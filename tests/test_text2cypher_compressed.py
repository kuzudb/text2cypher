...
@pytest.fixture
def compressed_schema(db_manager):
    compressor = SchemaCompressor(db_manager)
    return compressor.compress_schema()
...
def test_suite_1a(q, db_manager, compressed_schema):
    _run_test_suite(db_manager, compressed_schema, [q])


@pytest.mark.parametrize("q", suite_1b)
def test_suite_1b(q, db_manager, compressed_schema):
    _run_test_suite(db_manager, compressed_schema, [q])


@pytest.mark.parametrize("q", suite_1c)
def test_suite_1c(q, db_manager, compressed_schema):
    _run_test_suite(db_manager, compressed_schema, [q], expected_boolean=True)
